...
function LoadDataTable() {
    ...
    $.ajax({
        ...
        cache: false,
        data: "{}",
        url: "../api/attendance/LoadSession",
        success: function (data) {
            ...
            $('#table tbody').on('click', "#btnEdit", function () {
                ...
                id = Data["sessid"]
                document.getElementById("MainContent_txtsession").value = Data["session"];

                document.getElementById("MainContent_Sessid").value = id;



                Mainform.Sessid.value = id
            ...
            $('#table tbody').on('click', "#btnDelete", function () {
                ...
                var Data = table.row(currentrow).data();
                ID = Data["sessid"]
                var name = Data["session"];



                var Result = confirm("You are about to delete " + name + " Session")
                if (Result) {
...
function DeleteItem(ID) {
    ...
    $.ajax({
        ...
        cache: false,
        data: "{}",
        url: "../api/attendance/DeleteSession?id= " + ID + "",
        success: function (data) {
